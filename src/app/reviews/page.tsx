...
const TableThree = () => {
  ...
  const userDetail = async () => {
    ...
    try {
      ...
      setUserData(response.data.reviewData);
    } catch (error) {
  ...
  const getUserDetailsById = async (_id: string) => {
    ...
    try {
      const response = await axios.get(`/api/review/getReviewById/${_id}`);
          ...
          <div className="relative mt-16 max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white p-4 shadow-lg">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <p className="text-gray-800">{userDetails.product}</p>
              </div>
              <div>
                <label className="block font-semibold text-black">
                  Comments:
                </label>
                <p className="text-gray-800">{userDetails.comments}</p>
              </div>
              <div>
                <label className="block font-semibold text-black">
                  Rating:
                </label>
                <p className="text-gray-800">{userDetails.rating}</p>
              </div>              
              <div>
